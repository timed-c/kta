...
let rec eval_inst m instlst env  =
  ...
  | IBinOp(id,bop,ty,v1,v2)::lst -> 
    ...
    if !_debug then
      ...
      pprint_val nval ^. us" " ^. MlvmPPrint.llbinop bop);
    eval_inst m lst env'        
  (** IAlloc **)
  | IAlloca(id,ty,_)::lst ->            
    ...
    if !_debug then 
      ...
      ustring_of_int (List.length (mklst ty 1 [])) ^. us" ty=" ^. MlvmPPrint.lltype ty);
  ...
  | IGetElementPtr(id,ty,ptr,indices)::lst -> 
    ...
    let nval = (
      ...
      | VPtr(idx,arr) ->         
        ...
        if !_debug then 
          uprint_endline (us"DEBUG: %" ^. ustring_of_sid id ^. us"=" ^.
          ustring_of_int idx' ^. us" getelementptr" ^. us" ty=" ^. MlvmPPrint.lltype ty);
        VPtr(idx', arr)
  ...
  | ICall(id_opt,tail,ty,f,args)::lst -> 
    ...
    let LLFunc(_,params,blocks) = get_func_from_id f m in (
