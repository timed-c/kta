...
let make instlst used_outside = 
  let id_use_count = LlvmUtils.count_ids_uses instlst in
  let (rev_forest,_) = List.fold_left (fun (acc_forest,tmptree) inst ->
    ...
    ) ([],tmptree) (LlvmUtils.using_explst_in_instruction inst) in 
    let exp = TExp(inst,List.rev rev_childlist) in
    match LlvmUtils.defining_id_in_instruction inst with
