...
let compile_file filename fname args optimize debug bsconfig =
  try
     ...
     let program_code = MipsCfg.pprint_ocaml_cps_from_cfgmap false [] true fname cfgmap prog in

     let stdout = MipsSys.wcet_compile fname debug None bsconfig false program_code args in
