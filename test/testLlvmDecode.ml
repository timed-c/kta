...
let main = 
  let _ = MlvmUtils.const_int_val 0 0 in
...
(*
  ...
  let ast = LlvmDecode.bcfile2ast "ccode/functioncalls.bc" in
  (* uprint_endline (LlvmPPrint.pprint_module ast);  *)
  ...
  let ast = LlvmDecode.bcfile2ast "ccode/arrays.bc" in
  (* uprint_endline (LlvmPPrint.pprint_module ast); *)
  ...
  let ast = LlvmDecode.bcfile2ast "ccode/structs.bc" in
  (* uprint_endline (LlvmPPrint.pprint_module ast); *)
...
(*
  let ast = LlvmDecode.bcfile2ast "ccode/switches.bc" in
  uprint_endline (LlvmPPrint.pprint_module ast); 
